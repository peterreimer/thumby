...
p. To turn on disk caching (default is in memory caching of thumbnails) rename the conf/ehcache.xml.tmpl to conf/ehcache.xml

bc.. mv conf/ehcache.xml.tmpl conf/ehcache.xml

p. Make sure that maxBytesLocalHeap="512M" is not set to a greater value than your current jvm settings (java option -Xmx).
On a linuy system the provided template will store your thumbnail data under /tmp/thumby.cache/play.data.
